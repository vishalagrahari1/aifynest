...
const runMigration = async () => {
  ...
  for (const user of usersList) {
    ...
    // Verify profile exists in public.profiles (trigger should handle it, but insert if missing)
    ...
    if (!profileCheck) {
      ...
      if (insertError) {
        console.error(`Error inserting missing profile for ${user.email}:`, insertError.message);
      }
    } else {
  ...
  for (const rev of reviewsList) {
    ...
    const mappedUserId = userMap[rev.userId] || userMap['user-id'] || Object.values(userMap)[0];
    ...
    const { error } = await supabase.from('reviews').insert({
      ...
      content: rev.comment || rev.title || '',
      rating: rev.rating,
      ease_of_use: rev.ratingDimensions?.easeOfUse || rev.rating,
      value_for_money: rev.ratingDimensions?.valueForMoney || rev.rating,
      features: rev.ratingDimensions?.features || rev.rating,
      performance: rev.ratingDimensions?.performance || rev.rating,
