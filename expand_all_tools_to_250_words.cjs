...
// 220+ word rich descriptions for all seed tools
...
In addition to core image creation, Ideogram features an active community platform where creators can explore trending prompts, remix existing community designs, and discover inspiration for commercial advertising campaigns. Whether you need custom typography for event posters, branded merchandise graphics, or eye-catching editorial illustrations, Ideogram AI provides an intuitive web interface and powerful developer API endpoints to generate high-resolution, typography-rich imagery rapidly.`,
  ...
  "radarkit-ai": `RadarKit AI is an intelligent ad tracking, competitor intelligence, and marketing analytics platform engineered specifically for e-commerce store owners, media buyers, and digital advertising agencies. In today's hyper-competitive online retail ecosystem, discovering winning ad creatives and monitoring market trends before competitors is essential for business survival. RadarKit AI automates the continuous collection and algorithmic analysis of thousands of active digital ad campaigns across Facebook, TikTok, Instagram, and Google Ads.

By leveraging proprietary machine learning algorithms, RadarKit AI identifies viral ad creatives, high-converting copy structures, engagement metrics, and winning audience hooks in real time. Marketers can search through competitor ad libraries, analyze video duration and thumbnail strategies, and receive instant automated alerts when competitor ad campaigns scale aggressively. This data-driven approach removes guesswork from media buying budgets and enables brands to model proven creative concepts quickly.

Additionally, RadarKit AI features an AI-assisted creative generator that analyzes high-performing ads to produce tailored ad scripts, headlines, and call-to-action variants tailored to specific e-commerce product niches. Whether you are running Shopify dropshipping stores or managing multi-client ad budgets, RadarKit AI equips marketing teams with the strategic data insights required to maximize Return on Ad Spend (ROAS) and scale profitable advertising campaigns.`,

  "chatgpt": `ChatGPT is OpenAI's flagship conversational artificial intelligence assistant, built upon advanced large language models including GPT-4o and specialized reasoning architectures like o1 and o3. As a global leader in generative AI technology, ChatGPT provides millions of users worldwide with instant answers, writing assistance, programming support, complex mathematical problem-solving, and creative brainstorming capabilities through an intuitive, accessible chat interface.

The platform excels at an extraordinarily diverse range of tasks, from drafting long-form essays, email campaigns, and executive business reports to analyzing uploaded spreadsheets, debugging code, and synthesizing complex scientific datasets. With features such as Advanced Voice Mode, real-time web browsing synthesis, custom GPT creation, and DALL-E 3 image generation integrated directly into the workspace, ChatGPT serves as a versatile all-in-one digital assistant for students, software engineers, academic researchers, and enterprise teams alike.

ChatGPT offers tailored subscription tiers—including ChatGPT Plus, Team, and Enterprise—delivering higher usage caps, faster response speeds, enhanced data privacy guarantees, and early access to cutting-edge OpenAI research models. Whether utilized for daily administrative productivity, pair programming, multi-language translation, or strategic decision-making, ChatGPT remains an indispensable tool driving the modern artificial intelligence revolution.`,

  "midjourney": `Midjourney is an industry-leading generative artificial intelligence program celebrated worldwide for producing hyper-realistic, photorealistic, and visually stunning digital artwork from textual descriptions. Operating via Discord and an advanced dedicated web platform, Midjourney is widely used by concept artists, video game developers, fashion directors, digital marketers, and creative enthusiasts seeking unparalleled visual quality and atmospheric aesthetic depth.

The platform offers a deep suite of creative controls, allowing users to fine-tune image resolution, aspect ratios (--ar), camera angles, lighting conditions, and artistic stylization parameters (--stylize). Features such as Style Reference (--sref), Character Consistency (--cref), inpainting (Vary Region), and outpainting (Pan and Zoom) provide creators with granular control over generated compositions, ensuring visual continuity across multi-image advertising campaigns and storyboards.

With the release of Midjourney v6 and v6.1, the engine delivers enhanced prompt comprehension, realistic text rendering within complex scenes, micro-texture details, and lifelike human features. From creating cinematic concept art and high-fashion editorial imagery to designing website background patterns and physical product prototypes, Midjourney continuously sets the gold standard for text-to-image generative synthesis globally.`,

  "synthesia": `Synthesia is the market-leading enterprise AI video generation platform that enables organizations to turn plain text scripts into professional, studio-quality videos in over 140 languages without cameras, studio equipment, microphones, or actors. Founded by researchers from UCL, Stanford, TUM, and Cambridge, Synthesia streamlines video production workflows for corporate training, customer support onboarding, sales enablement, and localized marketing campaigns.

The platform provides access to over 230 diverse, hyper-realistic AI avatars created from real human actors, as well as the capability to generate custom personal AI avatars in minutes. Users simply type or paste their video script, select an avatar and voiceover accent, customize the canvas with brand colors, logos, and screen recordings, and generate high-definition MP4 videos automatically in a matter of minutes.

Synthesia's enterprise-grade features include seamless LMS integration, multi-user workspace collaboration, SOC 2 compliance, and automated video translation engines. Companies can easily update existing video content in real time by editing text scripts rather than re-shooting footage, reducing video production costs by up to 80% while dramatically accelerating global content localization speed.`,
...
Cursor supports top AI reasoning models including Claude 3.5 Sonnet, GPT-4o, and custom fine-tuned code completion models. Operating seamlessly across Windows, macOS, and Linux, Cursor offers custom privacy controls guaranteeing that user code remains confidential. For individual developers and engineering teams striving for hyper-productive coding workflows, Cursor represents the future of AI pair programming.`,
  ...
  "jasper": `Jasper is an enterprise-grade AI marketing platform built to help brand marketing teams, copywriters, and digital agencies generate high-performing, brand-aligned content at scale. Unlike general-purpose AI chat assistants, Jasper is specifically trained on high-converting marketing frameworks, direct response copy principles, and enterprise brand voice guidelines to deliver consistent, polished marketing assets.

The platform features an extensive library of over 50 marketing templates—including long-form blog post generators, Facebook & Google ad copy creators, email newsletter drafters, and e-commerce product description builders. Jasper’s Brand Voice technology scans your company's existing content to learn your brand's unique tone, vocabulary, and style guidelines, ensuring that every piece of generated content matches your brand identity perfectly.

Integrated with Surfer SEO for real-time keyword optimization and Grammarly for grammar perfection, Jasper enables marketing teams to accelerate campaign launches by up to ten times. With features like multi-user team collaboration workspaces, AI image generation, and a powerful browser extension, Jasper empowers marketers to scale content production across every digital channel effortlessly.`,
...
The platform supports advanced data processing capabilities, including cohort analysis, sentiment classification, missing value imputation, and financial modeling. With robust export options to Python notebooks, PDF summaries, and interactive web dashboards, Julius AI acts as a 24/7 personal data scientist that turns raw numbers into actionable business insights.`,
  ...
  "elevenlabs": `ElevenLabs is the industry-standard AI voice research company and software platform delivering state-of-the-art text-to-speech synthesis, voice cloning, and audio dubbing capabilities. Engineered with deep neural network architectures, ElevenLabs generates human-like speech with unmatched emotional depth, intonation control, natural pacing, and contextual awareness across 30+ global languages.

The platform's flagship feature set includes the Voice Library—a massive repository of community-contributed AI voices—and instant Voice Cloning, which creates a precise digital clone of any speaker from just a few seconds of clean audio reference. Content creators, audiobook publishers, game developers, and video production studios utilize ElevenLabs to produce immersive voiceovers, dynamic video game NPC dialogue, and localized foreign audio tracks.

Additionally, ElevenLabs provides AI Dubbing to translate video and audio content automatically while preserving the original speaker's unique voice characteristics and cadence. With flexible developer API access, granular latency controls, and enterprise-grade voice security protocols, ElevenLabs leads the global market in realistic AI voice generation technology.`,

  "phind": `Phind is an AI-powered search engine and pair programmer optimized specifically for software developers, system engineers, and technical researchers. Built to answer complex technical questions with speed and precision, Phind searches the web in real time, ingests technical documentation, scans Stack Overflow threads, and synthesizes clear, working code solutions accompanied by step-by-step technical explanations.

Unlike generic search engines or standard AI chat interfaces, Phind prioritizes code correctness and current software library syntax. Developers can input complex compiler error stack traces, request full component implementations, or ask for framework migration guides. Phind's proprietary models evaluate multiple online code sources, generate fully formatted code blocks, and cite authoritative documentation links for immediate verification.

Phind offers an interactive VS Code extension that brings its search and pair-programming capabilities directly into the code editor workspace. With features such as codebase context inclusion, customized search filters, and lightning-fast response times, Phind helps software developers overcome technical blockers and ship clean code faster.`,
...
Conversely, the tool features a reverse translation mode that decodes complex youth slang into clear, formal English. Whether you are crafting viral social media posts, analyzing youth cultural trends, or simply trying to understand online meme conversations, Gen Z Translator delivers accurate, culturally relevant text translations effortlessly.`,
  ...
  "lynote": `Lynote is an AI-powered research, note-taking, and knowledge management application engineered to capture, organize, and synthesize information effortlessly. Designed for university students, academic researchers, journalists, and knowledge workers, Lynote transforms fragmented handwritten notes, meeting audio recordings, PDF documents, and web articles into a structured, searchable personal knowledge graph.

The application automatically records and transcribes audio notes, highlights key thematic points, extracts actionable tasks, and generates concise executive summaries. Featuring semantic search and instant document Q&A, users can converse directly with their research library, asking natural language questions to recall specific quotes, statistical data points, or meeting decisions without manual file browsing.

Lynote seamlessly synchronizes across desktop and mobile devices, offering offline support, markdown formatting, export capabilities to Notion and Obsidian, and collaborative workspace sharing. By removing manual note organization and summarizing complex research documents, Lynote empowers professionals to focus on high-level strategic analysis and deep creative work.`,

  "anirole-ai": `Anirole AI is an immersive conversational AI platform and creative character studio specialized in anime roleplay, interactive storytelling, and virtual companion experiences. Designed for anime fans, creative fiction writers, and roleplay enthusiasts, Anirole AI allows users to chat with thousands of pre-created anime personas or craft custom AI characters with detailed backstories, unique dialogue styles, and visual avatars.

The platform utilizes fine-tuned language models optimized for narrative depth, emotional expression, and long-term memory retention. Users can embark on rich text-based roleplay quests, engage in casual daily conversations, or co-author interactive light novels. Custom character creation tools enable users to define specific personality traits, scenario settings, initial greeting messages, and visual artwork styles effortlessly.

Anirole AI prioritizes user privacy and customization, offering private chat rooms, custom scenario builders, and multi-character group chat modes. Whether you want to practice creative dialogue writing, explore fantasy worlds, or design your dream anime companion, Anirole AI offers a dynamic and highly entertaining roleplay environment.`,

  "pixaryai": `PixaryAI is a versatile AI photo generator, digital portrait studio, and image editing application engineered to transform ordinary photos into high-definition digital artwork, artistic avatars, and stylized visual graphics. Created for social media influencers, digital creators, and photography enthusiasts, PixaryAI turns simple text prompts and uploaded photos into striking visual compositions.

The platform features a wide range of artistic styles—including hyper-realistic photography, 3D digital art, anime aesthetic, vintage oil paintings, cyberpunk illustration, and cinematic concept art. Users can upload selfie portraits to generate professional business headshots, stylized social media avatars, or creative digital portraits in a matter of seconds.
...
Designed with a user-friendly interface, FixArt AI supports batch video processing, custom export codecs, and fast cloud rendering pipelines. Whether you are remastering historical video archives, optimizing video clips for high-definition displays, or enhancing social media video edits, FixArt AI delivers professional visual restoration.`,

  "moxt": `Moxt is an AI-powered business workflow automation, project tracking, and operations intelligence platform designed to streamline corporate task management and operational productivity. Built for modern hybrid teams, tech startups, and enterprise operations managers, Moxt integrates with existing software stacks to automate repetitive administrative workflows and provide real-time operational insights.
...
Moxt provides predictive timeline forecasting, workload distribution monitoring, and automated compliance auditing tools. By reducing manual status meetings and administrative overhead, Moxt helps operational leaders scale team efficiency and ensure project delivery consistency across the entire organization.`,
...
MiocAI incorporates built-in plagiarism checking, SEO keyword optimization recommendations, and multi-language translation capabilities. Whether drafting weekly marketing newsletters, polishing academic essays, or creating social media content schedules, MiocAI equips creators with a reliable and versatile writing companion.`,

  "trustmrr": `TrustMRR is a verified SaaS revenue tracking, financial analytics, and Monthly Recurring Revenue (MRR) transparency platform designed for software founders, angel investors, and software acquisition buyers. In the fast-moving SaaS ecosystem, TrustMRR delivers audit-grade financial data by integrating directly with payment processors like Stripe, Paddle, and ChartMogul.
...
HiAPI features real-time token usage analytics, unified billing dashboards, latency benchmarking, and enterprise security compliance. By eliminating vendor lock-in and reducing infrastructure maintenance overhead, HiAPI empowers engineering teams to build resilient AI applications faster.`,
...
Users can input detailed character profiles, relationship dynamics, narrative tropes, and plot outlines to generate custom scene continuations. The platform offers customizable content parameters, sensory detail controls, and emotional tone settings, allowing authors to tailor generated text to specific romance subgenres—such as contemporary, fantasy, historical, or paranormal romance.

Prioritizing complete user privacy and data security, AI Erotic Smut guarantees that all generated stories, prompts, and drafts remain 100% confidential. Whether drafting romance ebooks, writing fan fiction, or exploring creative story concepts, the tool provides a flexible and private narrative workspace.`,
...
async function processAllTools() {
  console.log('🔍 Transpiling and loading seedData.ts...');
  const seedPath = path.join(__dirname, 'src', 'utils', 'seedData.ts');
  const code = fs.readFileSync(seedPath, 'utf8');

  // Verify word counts for all expanded descriptions in our map
  console.log('--- WORD COUNT AUDIT FOR EXPANDED MAP ---');
  let shortInMap = 0;
  for (const [slug, desc] of Object.entries(toolDescriptions)) {
    const wc = getWordCount(desc);
    if (wc < 200) {
      console.error(`❌ [${slug}] Description is only ${wc} words!`);
      shortInMap++;
    } else {
  ...
  if (shortInMap > 0) {
    console.error(`Terminating because ${shortInMap} descriptions are under 200 words.`);
    return;
  }

  // Use JS evaluation of transpile to update seedData objects directly and format AST back
  ...
  // Update initialTools in memory
  let updatedToolsCount = 0;
  seedTools.forEach(t => {
    const cleanSlug = t.slug;
    if (toolDescriptions[cleanSlug]) {
      t.description = toolDescriptions[cleanSlug];
      updatedToolsCount++;
    }
  });

  // Re-serialize seedData.ts carefully
  // We can locate `export const initialTools: Tool[] = [` and replace the content up to `];`
  ...
  if (startIndex === -1) {
    console.error('Could not find start of initialTools array in seedData.ts!');
    return;
  }

  // Find matching closing bracket for initialTools
  // Find index of `export const initialReviews` or `export const initialCollections` or similar after initialTools
  const nextExportIndex = code.indexOf('export const initialReviews', startIndex);
  if (nextExportIndex === -1) {
    console.error('Could not find end marker after initialTools!');
    return;
  }
  ...
  console.log(`\n✅ Updated seedData.ts file with expanded descriptions for ${updatedToolsCount} tools!`);

  // Update Supabase DB
  console.log('\n🔄 Updating Supabase DB tools descriptions...');
  let dbSuccess = 0;
  for (const t of seedTools) {
    ...
    if (error) {
      console.warn(`⚠️ Supabase update warning for [${cleanSlug}]:`, error.message);
    } else {
      dbSuccess++;
    }
  }

  console.log(`\n🎉 Updated ${dbSuccess} tools in Supabase DB!`);
}
